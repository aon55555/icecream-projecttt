...
if(isset($_POST["category"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>Price range</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$cid = $row["cat_id"];
			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'>฿ $pro_price.00 /ชิ้น
				...
				if (isset($_SESSION["uid"])) {
					...
					echo '
						...
						<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
							...
							$x=0;
							$sql = "SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id ";
							$query = mysqli_query($con,$sql);
							while($row=mysqli_fetch_array($query)){
								$x++;
								echo  	
							...
							echo   
								'<input type="hidden" name="return" value="http://localhost/icecream-project/payment_success.php"/>
					                <input type="hidden" name="notify_url" value="http://localhost/icecream-project/payment_success.php">
									<input type="hidden" name="cancel_return" value="http://localhost/icecream-project/cancel.php"/>
