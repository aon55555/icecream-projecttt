...
class Products
{
	...
	public function deleteCategory($cid = null){
		if ($cid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Price rang removed'];
			}else{
				return ['status'=> 202, 'message'=> 'Failed to run query'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'Price rang id'];
		}
	...
	public function updateCategory($post = null){
		...
		if (!empty($cat_id) && !empty($e_cat_title)) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Price rang updated'];
			}else{
				return ['status'=> 202, 'message'=> 'Failed to run query'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'Invalid Price rang id'];
		}
